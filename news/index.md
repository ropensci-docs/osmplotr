...
- Remove all remaining support for the obsolete ‘sp’ package.
- ‘extract_osm_objects’ no longer has an ‘sf’ parameter and always
  returns ‘sf’ objects.

### Minor changes

- Replace now obsolete ‘spatstat.core’ to ‘spatstat.explore’ dependency
- Minor bug fix in internal `default_size()` function.
...
- ‘add_osm_surface’ functions changed to directly calculate and plot
  colours of objects, rather than rely on
...
- New function ‘osm_line2poly’ enables plotting polygonal shapes
  delineated only by lines, through tracing around the bounding box to
  form full polygons.
- New vignette to describe this functionality, “maps-with-ocean”.

### Minor changes

- ‘osm_basemap’ now accepts an ‘sf’ object instead of explicit ‘bbox’
  values, and extracts the corresponding ‘bbox’ directly from that
  object.
...
- Package is now also ‘sf’-compatible: objects to be plotted can be
  either ‘sp’ or ‘sf’ format, with all ‘osmplotr’ functions defaulting
  to ‘sf’

### Minor changes

- Title in DESCRIPTION changed from “Customisable Images of
  OpenStreetMap Data” to “Bespoke Images of ‘OpenStreetMap’ Data”
...
- ‘connect_highways’ also entirely re-coded to be much more efficient,
  but this should not affect functionality at all.
...
- ‘add_osm_surface’ did not previously work properly for different
  bboxes (and so zooming was not possible). Now fixed.
- both ‘add_osm_surface’ and ‘add_osm_groups’ now enable maps to be
  zoomed
- fix make_osm_map to produce maps even when not all requested data
  exists
...
- ‘print_osm_map’ added to enable device proportions to be automatically
  scaled to bounding boxes.
...
Major update with (almost) all plotting routines shifted from
‘graphics::plot’ to ‘ggplot2’. All previous parameters specifying
graphics devices (such as heights and widths) no longer apply.
...
- ‘extract_highway’, ‘extract_highways’, ‘order_lines’ no longer
  exported
...
- many tests added
- Change to ‘ggplot2’ has considerably changed structure of many
  functions. For details see function examples and vignettes
...
- added ‘add_osm_surface’ to spatially interpolate continuous surfaces
...
- all usages of ‘xylims’ (vectors of four components) and ‘get_xylims’
  changed to ‘bbox’ (2-by-2 matrices) for consistency with sp and tmap
- reduce size of ‘london’ data (through smaller bbox), with
  corresponding changes in vignettes
