...
The `adjust_colours` function allows different groups to be highlighted
...
![](data-maps_files/figure-html/unnamed-chunk-11-1.png)
...
![](data-maps_files/figure-html/unnamed-chunk-13-1.png)
...
Note both that when `add_osm_groups` is called with `colmat = TRUE`,
...
`connect_highways` returns a matrix of coordinates representing the
...
are `sf` data frames of class `sfc_LINESTRING`:
...
![](data-maps_files/figure-html/unnamed-chunk-18-1.png) This is because
`add_osm_surface` creates new `ggplot2` aesthetic schemes for each kind
of object, and these schemes are not intended to be modified or replaced
within a single plot. The above map may still be printed, but the
warning means that the last provided colour scheme will be applied to
all objects of that class. This means that `osmplotr` can only overlay
two distinct colour schemes: one for all line objects, and a potentially
different one for all polygon objects.
...
To illustrate, trim the above data to within a circular range of the
centre of the map.
...
passed, with these also determining the shapes of points, as illustrated
in the next example, which overlays trees on the map.
...
```
    ...
    ## Warning: Removed 4186 rows containing missing values or values outside the scale range
    ## (`geom_path()`).

    ## Warning: Removed 4161 rows containing missing values or values outside the scale range
    ## (`geom_path()`).
